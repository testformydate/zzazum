...
import com.mydate.zzazum.location.repository.LocationUpdateDao;
...
public class LocationDataService{
	...
	@Autowired
	private LocationUpdateDao apiUpdateDaoInter;
...
//		        System.out.println(url);
	        ...
	        for(int i = 0; i < numOfRows; i++){
	        	LocationVo loca = new LocationVo();
	        	loca.setP_no(items.getJSONObject(i).getInt("contentid"));
	        	loca.setP_name(items.getJSONObject(i).getString("title"));
	        	loca.setP_addr(items.getJSONObject(i).getString("addr1"));
	        	loca.setP_lat(items.getJSONObject(i).getDouble("mapy"));
	        	loca.setP_lng(items.getJSONObject(i).getDouble("mapx"));
	        	location.setP_image(items.getJSONObject(i).getString("firstimage"));
	        	b = apiUpdateDaoInter.insertApiData(loca);
